...
# -----------------------------------------------------------------------------
# HARDCODED KEYS (for local testing only—do not use this in production)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Simple in-memory cache (for production, consider a robust cache like Redis)
# -----------------------------------------------------------------------------
CACHE: Dict[str, Any] = {}
...
# Configuration
# -----------------------------------------------------------------------------
# Updated VALID_DATE_RANGES includes additional options:
...
def fetch_alpha_vantage_data(symbol: str, period: str = "10y") -> pd.DataFrame:
    if not ALPHA_API_KEY:
        ...
        return pd.DataFrame()
    outputsize = "full" if period in ["10y", "5y"] else "compact"
    url = f"https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol={symbol}&outputsize={outputsize}&apikey={ALPHA_API_KEY}"
    try:
...
# -----------------------------------------------------------------------------
# 3) Main Data Fetch: Yahoo Finance → Alpha Vantage → Finnhub
# -----------------------------------------------------------------------------
...
def backtest_model(model: Any, X: Any, y: Any, scaler: Any) -> Tuple[float, float]:
    return 0.0, 0.0  # Placeholder for LSTM backtesting
...
# -----------------------------------------------------------------------------
# Dash App Layout using Bootstrap Components
# -----------------------------------------------------------------------------
